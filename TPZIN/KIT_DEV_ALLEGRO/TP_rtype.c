	...
	while(playing) {
		...
		if(ev.type == ALLEGRO_EVENT_TIMER) {
            ...
            else if(gametype == 0) //JOGO
			{
				...
				//colisaoTiraoEnemy(&extraTiro, enemy, NUM_ENEMIES, &nave);
				//TIRAO + BLOCO
				...
				if(!pause){
					...
					atualizaExtraTiro(&extraTiro, nave); //passar timer
					desenhaExtraTiro(extraTiro);
		...
		//ev de pressionr tecla
			...
			else if(ev.keyboard.keycode == ALLEGRO_KEY_M && !ev.keyboard.repeat){
				if(gametype == 0 && !pause)
					atiraExtraTiro(&extraTiro, nave);
			}
		}
